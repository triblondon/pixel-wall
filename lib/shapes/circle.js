...
class Circle extends shape_1.default {
    constructor(options) {
        ...
        const row = Array((plotRadius * 2) + smoothing).fill(undefined).map(() => [255, 0, 0, 0]);
        ...
        if (smoothing === 1) {
            ...
            this.pixels = Array((radius * 2) + 1).fill(undefined).map((_, y) => Array((radius * 2) + 1).fill(undefined).map((_, x) => {
                let groupSum = [0, 0, 0, 0];
